...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    # image = image.astype(np.uint8)# Ensure dtype is uint8
...
#                     yap = (-1 / m) * (xap - xm) + ym
#                 except:
#                     xap, yap = xm, ym  # Default to midpoint if calculation fails

#             # Draw circle at midpoint
#             cv2.circle(image, (int(xm), int(ym)), radius, (195, 255, 62), 15)
...
#             keyinput.release_key('w')
#             keyinput.press_key('s')
#             cv2.putText(image, "Moving Back", (50, 50), font, 1.0, (0, 255, 0), 2, cv2.LINE_AA)
